  ...
  return {
    plugins: [
      ...
      basicSsl(),
      {
        ...
        transformIndexHtml(html) {
          // In production, we inject the strict CSP meta tag.
          // In dev, we rely on the headers (which allow unsafe-inline).
          if (!isDev) {
            return html.replace(
              '<!-- CSP_PLACEHOLDER -->',
              `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self' https://api.the-odds-api.com wss: ws:; img-src 'self' data:; font-src 'self' data:; object-src 'none'; base-uri 'self';">`
            )
          }
          return html;
        }
    ...
    server: {
      ...
      headers: {
        ...
        // Dev server needs 'unsafe-inline' for HMR
