  ...
  return {
    ...
    server: {
      ...
      headers: {
        ...
        // Sentinel: Restrict script-src 'unsafe-inline' to development only.
        // In production, we block inline scripts to mitigate XSS.
        'Content-Security-Policy': `default-src 'self'; script-src 'self' ${isProd ? '' : "'unsafe-inline'"}; style-src 'self' 'unsafe-inline'; connect-src 'self' https://api.the-odds-api.com wss: ws:; img-src 'self' data:; font-src 'self' data:; object-src 'none'; base-uri 'self';`,
      },
      proxy: {
        '/api/kalshi': {
